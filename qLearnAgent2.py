...
class QLearnAgent2:
    ...
    def update_q_value(self, last_state_id, action, next_state_id, reward, next_map):
        ...
        print('current_map_id=',last_state_id)
        ...
        if rot_90_next_state_id in self.q_matrix.keys():
            max_next_state = sorted(self.q_matrix[rot_90_next_state_id].items(), key=operator.itemgetter(1), reverse=True)
            max_value = max_next_state[0][1]
        ...
        self.q_matrix[rot_90_state_id][rotate_action] += (self.alpha/self.action_count) * (reward + self.discount_factor * max_value
        ...
        self.q_matrix[rot_180_state_id][rotate_action] += (self.alpha/self.action_count) * (reward + self.discount_factor * max_value
        ...
        self.q_matrix[rot_270_state_id][rotate_action] += (self.alpha/self.action_count) * (reward + self.discount_factor * max_value
